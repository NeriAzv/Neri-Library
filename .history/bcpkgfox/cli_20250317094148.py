...
def main():
    ...
    if re.search(r"\.bextract_pdf\b"):
        imports.append("pymupdf")

    if re.search(r"\.invoke_api_\b"):
        imports.append("requests")

    if re.search(r"\.wait_for\b"):
        imports.append("pygetwindow")

    if re.search(r"\.move_mouse_smoothly\b") or \
       re.search(r"\.move_to_image\b"):
        imports.append("pyautogui")

    if re.search(r"\.initialize_driver\b"):
        imports.extend(["webdriver-manager", "undetected-chromedriver", "pyautogui", "psutil"])

    if re.search(r"\.stealth max\b"):
