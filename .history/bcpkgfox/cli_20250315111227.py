...
def main():
    ...
    try:
        with open(target_file, "rb") as file:  # Modo binário
            raw_data = file.read()
            file_content = raw_data.decode("utf-8", errors="replace")  # Tenta UTF-8 primeiro
    except Exception as e:
        try:
            file_content = raw_data.decode("latin-1")  # Fallback para Latin-1
        except:
            print(f"Falha crítica ao ler o arquivo: {e}")
            return
