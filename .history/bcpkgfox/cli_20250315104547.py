...
def main():
    ...
    codificacoes_tentativas = ['utf-8', 'latin-1', 'iso-8859-1', 'cp1252'] ; file_content = ""
    for codificacao in codificacoes_tentativas:
        try:
            with open(target_file, "r", encoding=codificacao) as file:
                file_content = file.read()
            break
        except Exception as e:
            print(f"Tentativa com codificação {codificacao} falhou: {e}")
            continue
    ...
    if len(re.findall(r"\.move_to_image\b", file_content)) >= 1:
