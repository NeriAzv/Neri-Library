...
def main():
    ...
    if len(re.search(r"\.bextract_pdf\b", file_content)) >= 1:
        imports.append("pymupdf")

    if len(re.search(r"\.invoke_api_\b", file_content)) >= 1:
        imports.append("requests")

    if len(re.search(r"\.wait_for\b", file_content)) >= 1:
        imports.append("pygetwindow")

    if len(re.search(r"\.move_mouse_smoothly\b", file_content)) >= 1 or \
       len(re.search(r"\.move_to_image\b", file_content)) >= 1:
        imports.append("pyautogui")

    if len(re.search(r"\.initialize_driver\b", file_content)) >= 1:
        imports.extend(["webdriver-manager", "undetected-chromedriver", "pyautogui", "psutil"])

    if len(re.search(r"\.stealth max\b", file_content)) >= 1:
