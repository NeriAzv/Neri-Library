...
class System:
    ...
    def extract_pdf(pdf_path: str = None) -> str:
        ...
        Attributes:
            pdf_path(str): The path of pdf file to extract.

        Note:
            Use resource_path to localize the pdf_path to not bug the pyinstaller."""

        try:
            if os.path.exists(pdf_path) and os.path.isfile:
            ...
            raise ValueError("File not found or not is a file")
        except Exception as e: raise ValueError(f"  {RD}>>> File Error!{RESET}") from e
