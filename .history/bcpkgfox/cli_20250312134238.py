...
def main():
    ...
    def animate_rgb_text(text, delay=0.01):
        ...
        hue = 0
        print(f"\n\n {ORANGE}>{RESET} Dependências do arquivo {DK_ORANGE}'{target_file}'{RESET} identificadas com sucesso")
        time.sleep(1)
        print(f"\n {ORANGE}PIP:{RESET}")
        time.sleep(0.5)
        while True:
    ...
    if imports:
        try:
            animate_rgb_text(f'pip install {", ".join(imports)}', delay=0.005)
        except KeyboardInterrupt:
            print(f"{ORANGE}>{RESET} Copie e cole no terminal. (obs: só identifica as libs que são pertencentes da lib bcfox) \n")
    else:
